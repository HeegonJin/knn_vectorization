...
def compute_distances_two_loops(x_train, x_test):
  ...
  for col in range(num_test):
      for row in range(num_train):
          dists[row][col] = ((x_train[row].flatten() - x_test[col].flatten()) ** 2).sum()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for row in range(num_train):
      dists[row] = torch.sum(((x_train[row].flatten().repeat(num_test) - x_test.flatten()).view(num_test, -1))**2, dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##################################x###########################################
  # Replace "pass" statement with your code
  sqr_train = ((x_train.flatten().view(num_train, -1)) ** 2).sum(dim=1).reshape(num_train, 1)
  sqr_test = ((x_test.flatten().view(num_test, -1)) ** 2).sum(dim=1).reshape(1, num_test)
  mm = torch.mm((x_train.flatten().view(num_train, -1)), (x_test.flatten().view(num_test, -1)).T)
  dists = sqr_train -2*mm + sqr_test
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for sample in range(num_test):
    col = dists[:, sample]
    values, idxs = col.topk(k, largest=False)
    y_pred.append(torch.bincount(y_train[idxs]).argmax())
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
      ...
      for fold in range(num_folds):
        x_train_set = torch.cat(x_train_folds[:fold] + x_train_folds[fold+1:])
        y_train_set = torch.cat(y_train_folds[:fold] + y_train_folds[fold+1:])
        classifier = KnnClassifier(x_train_set, y_train_set)
        accuracy = classifier.check_accuracy(x_train_folds[fold], y_train_folds[fold], k, quiet=True)
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  sorted_ = [k for k, v in sorted(k_to_accuracies.items(), key=lambda x : torch.mean(torch.tensor(x[1])))]
  best_k = sorted_[-1]
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
